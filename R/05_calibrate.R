...
CALIB_EXCLUDE <- c(
  ...
  Ogdensburg = "The city tax was re-imposed effective 1 March 2022 (DTF notice ST-22-1), so its jurisdiction code is new and its implied base share is still rising (3.01% -> 3.38% -> 3.45% of the county base over FY2023-25, against a flat ~47% for long-established Ithaca). Personal-vehicle border crossings at Ogdensburg (BTS) were RECOVERING over the same years (154k in 2022, 226k in 2023, 258k in 2024, against 292k in 2019), so part of the rise is traffic returning rather than the code propagating; an earlier reading of this project that traffic had collapsed was wrong. Albany would face the same new-code effect in its first years, so this is used, cautiously, for the first-year discount rather than for the steady state."
)
