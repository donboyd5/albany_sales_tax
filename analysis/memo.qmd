...
alloc_fits <- bind_rows(lapply(c("lodes_ex_pubadmin", "lodes", "ec_payroll"), function(ba) {
  cal_v <- build_calibration_variant(business_allocator = ba); f_v <- fit_calibration(cal_v)
...
rev_9261_as_business <- rev(apply_calibration(fit, B_pred + B_9261 * (a_wxp - a_mv))$B_c_central)
...
| **Upper case** | **\$`r fm(alloc_fits[["Revenue, $m"]][2] * 1e6)` million** if government jobs are counted in the allocator for business purchases (§9.1); the central case leaves them out because governments are exempt purchasers |
...
| **Largest uncertainty** | How to treat the city's very large public sector. Government purchases are exempt from sales tax, so government jobs are left out of the allocator for business purchases; counting them would add about \$`r fm(sens$Rev[2] - REV)` M. The 17 cities the method is checked against have too little government employment to settle it either way. |
...
1. **The treatment of Albany's exempt public sector.** Government is `r fp(pub_pct, 0)` of the county's jobs and `r fp(a_pub, 0)` of those are in the city; government purchases are exempt, so those jobs are excluded from the business allocator, but the only check available — the seventeen calibration cities — has too little government employment to confirm or refute the choice. Including them adds about \$`r fm(sens$Rev[2] - REV)` M. Hospitals and universities, also largely exempt, remain in the count. A DTF tabulation would resolve it.
