...
| **Statistical band** | 68%: \$`r fm(rev(est_pref$lo68))`–`r fm(rev(est_pref$hi68))` M; 90%: \$`r fm(rev(est_pref$lo90))`–`r fm(rev(est_pref$hi90))` M |
...
6. **Multiply by 0.5%**, adjust for the small wedge between tax on the base and cash actually distributed (φ, effectively zero), and allow for any shopping response to the higher rate (β, taken as none): **≈ \$`r fm(REV)` million a year**, plausibly \$10–15 million.
...
Two of the seventeen residuals have verifiable structural causes that do not apply to Albany, and §5A works through them along with every other city. Setting those two aside brings σ to `r f3(fit_pref$sigma)`, a one-σ miss of `r sd_up(fit_pref$sigma)` / `r sd_dn(fit_pref$sigma)`, and that is the fit used. Both are reported throughout.
...
changes the answer materially. It is defensible because the reasons were
established from outside sources — the Census Block Assignment Files and DTF's
own notice ST-22-1 — rather than from the size of the residual, and because
neither reason transfers to Albany. **Both fits are carried through everything
below**, and a reader who rejects the exclusions should read
\$`r fm(REV_all)` M with a 68% band of
...
1. Every calibration variant in §5.3 lands between \$`r fm(min(cv$rev))` M and \$`r fm(max(cv$rev))` M, and the choice of business allocator — the one modelling decision that matters — spans \$`r fm(min(alloc_fits[["Revenue, $m"]]) * 1e6)`–`r fm(max(alloc_fits[["Revenue, $m"]]) * 1e6)` M (§9.1).
...
2. **The method still misses by `r sd_up(fit_pref$sigma)` / `r sd_dn(fit_pref$sigma)` at one σ on the cities where it can be checked**, after the two structural exclusions, and by `r sd_up(fit$sigma)` / `r sd_dn(fit$sigma)` without them. Albany may be an easier case — large, fine NAICS detail, near-complete Economic Census coverage — but nothing proves it.
...
9. **NAICS 9261 is classified as residence-sourced vehicle tax on the strength of its statewide distribution**, not on documentation. If it is instead business activity sourced where it is reported, it belongs with the business class and the estimate rises by about \$`r fm(rev_9261_as_business - REV)` M.
