...
| **Reasonable planning range** | **\$8–15 million** — the envelope of every specification tried and of the band at the upper end of σ’s own confidence interval; §7 sets out the construction. Budget figure **\$11 million**, with a first-year haircut (§12). |
| **Statistical band** | 68%: \$9.2–13.5 M; 90%: \$8.1–15.3 M — at the point estimate of σ. σ itself is known only to within \[0.139, 0.309\] on 13 degrees of freedom; at its upper end the 68% band is \$8.2–15.2 M. |
...
6.  **Multiply by 0.5%**, adjust for the small wedge between tax on the base and cash actually distributed (φ, effectively zero), and allow for any shopping response to the higher rate (β, taken as none): **≈ \$11.1 million a year**, plausibly \$8–15 million.
...
One New York city already has a combined rate half a point above its county’s. Publication 718 gives 8⅜% for Westchester County and for its cities of Mount Vernon, New Rochelle and White Plains, but 8⅞% for Yonkers. The decomposition is:
...
The ⅜% Metropolitan Commuter Transportation District surcharge applies throughout Westchester and is *not* what makes Yonkers different. Yonkers fully preempts the county’s 4% — Westchester collects nothing inside the city — and imposes 4½%; the extra half point is the city’s rate exceeding the county’s. That is the only published instance of a 0.5-point intra-county rate differential in the state, and Yonkers appears below both as a calibration city and as the analogy for what such a differential looks like in practice — how it is administered and whether sales cross the line.

**It is an economic analogy, not a legal precedent, and the difference matters.** Yonkers’ 4½% is a 3% rate that preempts the county entirely — Westchester collects nothing inside the city and, per OSC’s Appendix C, distributes its shared revenue only to its towns, villages and its two *non*-preempting cities, Rye and Peekskill — plus 1½% of “additional” and “special” rates authorized by specific state legislation. Every city-specific rate in Publication 718-A that sits above the standard structure (Yonkers, Mount Vernon, New Rochelle, White Plains) has that shape: an add-on layered on top of preemption. What Albany would be asking for — keep the full population-based share of the county’s distribution *and* levy a standalone rate on the general base — has no example among New York’s general sales taxes that this analysis could find. The nearest things are the narrow-base city taxes that do coexist with an unpreempted county rate: Niagara Falls, Lockport and Long Beach tax admissions, restaurant meals, hotel occupancy or utilities, Newburgh and Port Jervis tax consumer utilities, and the Albany City School District’s own 3% utility tax is exactly such a layered tax inside Albany. Those are precedents for layering without preemption, but on selected services, not on the general base.

A legislator’s first question will therefore be why the city does not simply exercise the preemption right it already holds under §1224. §11 is the answer: with Albany’s share of the county base below its population share, preempting 1½ points would forfeit more in county distributions than it raises — about \$13.0 million a year on these estimates. The add-on is not a way around preemption; it is the only structure under which a city in Albany’s position gains anything at all.

Two things this memo does not do. It does not address the legal route to authorization beyond the observation above that a special act would be needed, as it was for every existing add-on; whether the Legislature would grant one to a non-preempting city is a political question outside this analysis. And it does not model incidence or economic effects beyond a simple behavioral factor.
...
|:---|---:|---:|---:|---:|---:|---:|---:|
| All 17 cities | 17 | 1.060 | 0.136 | 0.453 | 0.849 | 2.47 | 12.5 |
...
| Excluding Salamanca and Ogdensburg (used) | 15 | 0.881 | 0.033 | 0.192 | 0.954 | 2.21 | 11.1 |
...
Two of the seventeen residuals have verifiable structural causes that do not apply to Albany, and §5A works through them along with every other city. Setting those two aside brings σ to 0.192, a one-σ miss of +21% / -17%, and that is the fit used. That figure is under the plan’s 0.25 threshold, but the threshold was written for the full sample, and clearing it after exclusions is not the same as clearing it. Both fits are reported throughout.
...
This is a judgment, and it is the one place in this memo where dropping data changes the answer materially. The sequence should be stated honestly: the two cities were noticed *because* they were the largest residuals, and the reasons were then investigated. What makes the exclusions defensible is not the order of discovery but that the reasons come from sources outside this analysis — the Census Block Assignment Files for Salamanca; DTF’s notice ST-22-1 and the city code for Ogdensburg — and that neither transfers to Albany. Ogdensburg’s case is the weaker of the two, because part of its evidence (the rising implied share) is a pattern in the same data.

The check that a reader can audit is leave-one-out: refit dropping each city in turn and see which removals actually change σ.

| City dropped     |   n | Slope β | σ (logs) | Revenue, \$m |
|:-----------------|----:|--------:|---------:|-------------:|
| Ogdensburg       |  16 |   0.991 |    0.363 |         12.0 |
| Salamanca        |  16 |   0.977 |    0.387 |         11.7 |
| Gloversville     |  16 |   1.109 |    0.444 |         12.9 |
| Johnstown        |  16 |   1.091 |    0.452 |         12.7 |
| (none)           |  17 |   1.060 |    0.453 |         12.5 |
| Rome             |  16 |   1.063 |    0.456 |         12.2 |
| Ithaca           |  16 |   1.053 |    0.459 |         12.1 |
| Yonkers          |  16 |   1.113 |    0.459 |         13.8 |
| Norwich          |  16 |   1.093 |    0.460 |         12.8 |
| Oneida           |  16 |   1.075 |    0.461 |         12.5 |
| Glens Falls      |  16 |   1.057 |    0.466 |         12.6 |
| Auburn           |  16 |   1.062 |    0.468 |         12.6 |
| Mount Vernon     |  16 |   1.054 |    0.468 |         12.3 |
| Utica            |  16 |   1.055 |    0.468 |         12.3 |
| Saratoga Springs |  16 |   1.056 |    0.469 |         12.3 |
| Olean            |  16 |   1.059 |    0.469 |         12.5 |
| New Rochelle     |  16 |   1.062 |    0.469 |         12.5 |
| White Plains     |  16 |   1.059 |    0.469 |         12.4 |

Only the two excluded cities move σ appreciably; removing any other single city leaves it between 0.444 and 0.469. The exclusions are not one arbitrary pair among many.

**Both fits are carried through everything below**, and a reader who rejects the exclusions should read \$12.5 M with a 68% band of \$7.9–19.6 M instead.
...
The **reasonable planning range of \$8–15 million** is computed rather than chosen: it is the envelope of (i) the preferred 68% band evaluated at the *upper* end of σ’s 95% confidence interval, σ ∈ \[0.139, 0.309\], and (ii) every specification variant in §9 — exclusion set × functional form, business allocator, and calibration sample. Three further observations bear on how to read it:

1.  The statistical band understates the uncertainty in two ways that the envelope partly repairs. σ is estimated from 15 residuals on 13 degrees of freedom and is itself uncertain by a factor of about 1.6 either way. And §5A shows 83% of the residual variance is *between counties* — Albany is a fresh draw from a distribution of county effects estimated on only 11 counties, so the effective sample is nearer 11 than 15.
2.  The exclusion set and the functional form interact (§9.2): on the same 15 cities the slope-fixed fit gives \$13.1 M against the log-log’s \$11.1 M, and the choice of business allocator spans \$11.1–12.0 M (§9.1). These discrete choices sit outside the residual-σ concept entirely, which is why the planning range is built from them rather than from σ alone.
...
| Business allocator: employment excluding government, education and health (exempt-institution bound) | 2.685 | 2.079 | 10.5 | -6.0% |
...
**Hospitals and universities.** The same exemption logic applies to them, and LODES separates education (CNS15) and health care (CNS16), so a bound is available: dropping those sectors as well as government gives a city share of 35.6% and revenue of \$10.5 M. It is a bound rather than an estimate — the two sectors also contain taxable private clinics and schools — but it says the remaining exempt-institution exposure is worth at most about \$0.7 M.
...
**Exclusion set × functional form.** The log-log slope on the preferred sample is 0.881 (se 0.033), significantly below 1: larger predicted bases are corrected down proportionally more, and Albany’s raw prediction is the second largest in the set after Yonkers. A slope fixed at 1 — scaling the raw prediction by the geometric-mean observed/predicted ratio — removes that gradient:

| Sample | n | Log-log fit, \$m | Slope fixed at 1, \$m | Median ratio, \$m |
|:---|---:|---:|---:|---:|
| All 17 | 17 | 12.5 | 11.4 | 12.8 |
| Excluding Salamanca and Ogdensburg | 15 | 11.1 | 13.1 | 13.2 |

The two choices interact: on all seventeen the functional form barely matters, on the fifteen it is worth \$2.0 M. The size gradient is carried largely by the four Westchester cities, which are dense residential cities rather than state capitals; dropping them flips the slope above 1 and the estimate to \$15.3 M with a much worse fit. The log-log fit is used because it is the better-fitting specification on every sample, but the slope-fixed figure is the more conservative reading for a large city, and it is inside the planning range.
...
Validation 3 is the only place an allocator can be tested against a direct observation of an Albany city base. The measured share (48.9%) sits between the two employment allocators — and closer to total employment (52.9%) than to the private-sector allocator used (42.8%). That is not evidence for counting government jobs: the State’s own utility purchases are exempt and appear in neither the school-district collections nor the county’s taxable sales, so the measured figure is *taxable* commercial and residential load, which concentrates where the office buildings are rather than where the government payroll is. It does suggest the business-purchase share may sit somewhat above the allocator used, which is one reason the exempt-institution bound in §9.1 and the slope-fixed fit in §9.2 are both worth reading.
...
**A new jurisdiction code collects less than its base implies at first.** Ogdensburg’s implied share of its county base was 87% of its third-year level in year one and 98% in year two, and year three may not yet be the asymptote (§5A). That is one city, so it can frame a haircut but not fix it: **10–20% below steady-state in the first full year, tapering in the second**, is the range consistent with it. Albany would be a new code. Applied to the central estimate, the first *full* year is more like \$8.9–10.0 M before the cash-lag effect below, and the two effects compound.
...
1.  **Exempt institutions inside the city.** Government purchases are exempt and government jobs are excluded from the business allocator accordingly, but hospitals and universities — also largely exempt purchasers — remain in the count, and Albany has many of both. Dropping the education and health sectors too bounds the exposure at about \$0.7 M (§9.1); the true figure is smaller because those sectors also contain taxable private employers.
2.  **The two excluded calibration cities.** The preferred fit drops Salamanca and Ogdensburg for the reasons in §5A. Keeping them gives \$12.5 M with a 68% band of \$7.9–19.6 M. The leave-one-out table shows they are the only two cities whose removal changes the fit, but the decision remains a judgment.
3.  **The method still misses by +21% / -17% at one σ on the cities where it can be checked**, after the two structural exclusions, and by +57% / -36% without them. Albany may be an easier case — large, fine NAICS detail, near-complete Economic Census coverage — but nothing proves it.
...
10. **NAICS 9261 is classified as residence-sourced vehicle tax on the strength of its statewide distribution**, not on documentation. If it is instead business activity sourced where it is reported, it belongs with the business class and the estimate rises by about \$0.1 M.

## 14. Recommendations

1.  **Plan on \$8–15 million a year at steady state, with \$11 million as the budget figure**, at current activity levels, rising with the county base at 2–4% a year — and 10–20% less than that in the first full year while vendors’ systems pick up the new code (§12). Do not present the estimate without its range.
...
3.  **Prefer the add-on to preemption.** With Albany’s R at 0.77, below the 1.067 break-even, preemption loses about \$13.0 million a year relative to the existing distribution; the add-on has no such cost.
